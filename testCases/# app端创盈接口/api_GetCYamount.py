...
class GetCYamount(GesturePwdInit):
    """轻易创盈-获取理财金额接口测试"""
    def test_getCYamount_normal(self):
        """轻易创盈-获取理财金额正常流程"""
        # 获取token
        ...
        #     "pageSize":"10"
        # }
        _url = self.rf.get_option_value('app','host')+self.rf.get_option_value('api','getCYamount_url')
...
if __name__ == '__main__':
    suite = unittest.TestSuite(map(GetCYamount,[
        "test_getCYamount_normal",
    ]))
    ...
    fp = file(filename, 'wb')
    runner = HTMLTestRunner.HTMLTestRunner(stream=fp, title='轻易贷App创盈获取创盈投资金额详情测试报告',
                                           description="轻易贷App创盈获取创盈投资金额接口测试详情:")
    runner.run(suite)