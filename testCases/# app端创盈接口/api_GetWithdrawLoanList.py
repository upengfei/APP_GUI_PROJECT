...
class GetWithdrawloanList(GesturePwdInit):
    """轻易创盈-获取撤资审核中列表接口测试"""
    def test_getWithdrawloadList_normal(self):
        """轻易创盈-获取撤资审核中列表正常流程"""
        # 获取token
        ...
        _url = self.rf.get_option_value('app','host')+self.rf.get_option_value('api','getWithdrawloadList_url')
...
if __name__ == '__main__':
    suite = unittest.TestSuite(map(GetWithdrawloanList,[
        "test_getWithdrawloadList_normal",
    ]))
    ...
    fp = file(filename, 'wb')
    runner = HTMLTestRunner.HTMLTestRunner(stream=fp, title='轻易贷App创盈获取撤资审核列表详情测试报告',
                                           description="轻易贷App创盈获取创盈撤资审核中列表接口测试详情:")
    runner.run(suite)