...
class IUpdatelockdetail(unittest.TestCase):
    def setUp(self):
    ...
    # 正常流程
    def test_updatelockdetail(self):
        """ 修改锁定金额 -正常流程-修改金额"""
        # 获取用户userID
        ...
        url = self.rf.get_option_value("http","base_url")+self.rf.get_option_value("api","updatelockdetail_url")

        r = self.s.post(url,data=json.dumps(data),headers=header,verify=False)
        print "返回报文为：{0}".format(r.content)

        # 校验数据库lock_detail
        sql1 = 'select amount from lock_detail where user_id = "%s";'% self.userId
        self.md.execute(sql1)
        iamount = self.md.fetchone()[0]
        if int(iamount)==int(value[1]):
            logger.info("校验数据库成功，校验sql：%s,修改后的金额为:%s" % (sql1,iamount))
            print "校验数据库成功，校验sql：%s,修改后的金额为:%s" % (sql1,iamount)
        else:
            logger.error("校验数据库失败，校验sql：%s,修改后的金额为:%s" % (sql1,iamount))
            print "校验数据库失败，校验sql：%s,修改后的金额为:%s" % (sql1,iamount)
...
if __name__=='__main__':
    suite = unittest.TestSuite(map(IUpdatelockdetail,[
        "test_updatelockdetail",

    ]))
    now = time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime())
    HttpFunc.HttpFunc.create_report("updatelockdetail_report")

    filename = HttpFunc.HttpFunc.get_report("updatelockdetail_report")
    fp = file(filename, 'wb')
